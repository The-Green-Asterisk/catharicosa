<div class="m-10">
    ...
    @if ($toggle == 0)
        ...
        @endif
    @elseif ($toggle == 1)
        <p class="text-lg underline font-bold decoration-4 my-4">Inventory</p>
        <div class="flex flex-col space-y-4">
            ...
            <div x-data="{}" x-on:click="$wire.emit('showModal', '{{ $catName }}', {{ $item->id }})" class="cursor-pointer">
                ...
                <div class="shadow">
                    <div class="rounded-t border border-slate-500 bg-slate-200 border-b-none font-bold p-1 flex flex-row">
                        ...
                        <input type="image" src="images/trash.png" wire:click="deleteInvItem('{{ $item->id }}')" height="15px" width="15px" class="opacity-50 hover:opacity-100" title="Delete Item" />
                    </div>
                    <div class="rounded-b border border-slate-500 border-t-none p-2">
                        <p class="text-xs uppercase font-bold">Description:</p>
                        {!! $item->description !!}
